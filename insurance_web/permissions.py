...
def check_admin_permission(user):
    ...
    if not user.profile.is_admin():
        raise PermissionDenied("You do not have administrator permissions to access this resource.")
...
def check_not_self_action(user, target_user, action_name="perform this action"):
    ...
    if user.id == target_user.id:
        raise PermissionDenied(f"You cannot {action_name} on your own account.")
